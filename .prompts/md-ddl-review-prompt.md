...
- Does `md-ddl-specification/1-Foundation.md` contain a "Validation Model" section that defines the two-tier model, and does its pre-flight scope match the rule set in `guides/validation-tooling.md § Pre-Flight Check Scope` and the rules implemented in `src/md_ddl/lint.py`? All three must agree.
...
### Architectural Tenet Alignment

**Purpose:** Verify that spec rules trace to the architectural philosophy and that
the architecture skill's tenet list stays in sync.

**Review lens:**

- Do spec rules trace to at least one architectural tenet (listed in
  `agents/agent-architect/skills/architecture/SKILL.md`)? Flag rules that appear to
  have no architectural justification — they may be correct but should have
  documented rationale.
- Does the Agent Architect architecture skill's tenet list match the current
  architectural tenets? Flag drift between the tenet table in SKILL.md and the
  tenet summary in `.github/copilot-instructions.md`.
- Do the reference stubs in `agents/agent-architect/skills/architecture/references/`
  point to files that exist in `references/architecture/`?
- Are the Mermaid diagram inlines in blog posts consistent with the source files
  in `references/architecture/diagrams_converted/`?
