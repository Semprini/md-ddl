...
- **Pre-flight check scope creep risk.** The pre-flight set expanded from 5 checks to 12 rules when diagram/table/YAML consistency was folded in. Probe that expansion hard: is every rule genuinely detecting a model that contradicts itself, or has convention checking crept in under a reference-integrity label? What prevents a thirteenth rule being added casually? Is there a mechanism (spec version bump requirement) that makes scope expansion a deliberate decision rather than a silent accumulation? If the mechanism exists only as text in the spec, how would a contributor know to check?
- **Is the warning tier doing real work, or is it a loophole?** Rules that report warnings rather than errors were justified as "legitimate organisational deviation". Check whether that justification holds per rule, or whether the warning tier is being used to smuggle in convention checks the philosophy rejects.
...
### 10. Architectural Philosophy Alignment

The standard claims to implement the Data Autonomy architectural style, with 13
tenets documented in `agents/agent-architect/skills/architecture/SKILL.md`. Probe
whether this claim holds:

- Does the standard actually implement the stated architectural philosophy, or does
  it subtly contradict it? Identify spec rules that claim one tenet but effectively
  serve another, or rules that work against a tenet.
- Are there design decisions in the spec that serve implementation convenience but
  violate the tenets? For example, does any spec rule centralise what the tenets
  say should be distributed, or hard-code what the tenets say should be flexible?
- Could the architectural philosophy be used to justify removing or simplifying
  existing spec rules? Identify rules that survive only through inertia and could
  be pruned by applying the tenets rigorously.
- Is the tenet list itself internally consistent? Do any tenets contradict each
  other in practice (not just in theory)?
- Does the architecture skill present the tenets honestly, or does it subtly
  bias toward adoption? Check whether the counter-positions are genuinely fair
  or straw-man arguments.

### 11. Where Would AI Evaluation Miss the Problem?
